...
@alg.input(type=bool, name="maskNull", label="Mask FMask null pixels", default=True, advanced=True)
@alg.input(type=bool, name="maskCloud", label="Mask FMask cloud pixels", default=True, advanced=True)
@alg.input(type=bool, name="maskShadow", label="Mask FMask shadow pixels", default=True, advanced=True)
@alg.input(type=bool, name="maskSnow", label="Mask FMask snow pixels", default=True, advanced=True)
...
def burncloudmask(instance, parameters, context, feedback, inputs):
    ...
    params = {"il": [instance.parameterAsString(parameters, 'dataFile', context), reclassMask], "exp": "im1*im2b1", "out": instance.parameterAsString(parameters, 'outputFile', context)}
